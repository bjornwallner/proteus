...
##change these to the right location


#my $NCBI_DIR = "/proj/wallner/users/x_bjowa/local/blast-2.2.26/bin/";

#my $PSIPRED= "/proj/wallner/users/x_bjowa/local/psipred/";
#my $SEQ_DB = "/proj/wallner/users/x_bjowa/DB/latest/uniref90.fasta";
#if(!-e "$NCBI_DIR/blastpgp") {

my $DISOPRED_DIR=dirname(abs_path($0));
my $NCBI_DIR=abs_path("$DISOPRED_DIR/../blast-2.2.26/bin/");
my $PSIPRED=abs_path("$DISOPRED_DIR/../psipred/");
my $SEQ_DB=abs_path("$DISOPRED_DIR/../DB/uniref90.fasta");
#}  
#print "$NCBI_DIR\n$PSIPRED\n$SEQ_DB\n";
#exit;

my $SEQ_DB_DIR=dirname($SEQ_DB);
if(defined($ARGV[1])) {
    $SEQ_DB=$ARGV[1];
} 
...
#else {
#my $clean_up_tmp=0;
#    my $tmp=`echo \$SNIC_TMP`;
#    chomp($tmp);
#    if($tmp=~/scratch/) {
#	`mkdir $tmp/DB`;
#	print "Syncing $SEQ_DB_DIR to $tmp/DB/...\n";
#	`rsync --size-only -ar $SEQ_DB_DIR/ $tmp/DB/`;
#	$SEQ_DB="$tmp/DB/uniref90.fasta";
#	#$clean_up_tmp=1;
#    }
#}
...
my $EXE_DIR_PSIPRED ="$PSIPRED/bin";
my $DATA_DIR_PSIPRED = "$PSIPRED/data"; # the path of the data directory
...
die "[$0] ERROR: The command line should only contain the path to the input sequence file or run_only_blast_flag\n" if ( scalar @ARGV > 3);
...
my $psi_fn = join '/', $out_dir, $tmp_base.".psi";
my $args;
if(!-e $psi_fn) {
    print "Running PSI-BLAST search ...\n\n";
    print "... only\n" if($run_only_blast);
# run psiblast
    $args = join ' ', $NCBI_DIR."/blastpgp", "-i", $fasta_fn, "-d", $SEQ_DB, $PSIBLAST_PAR, "-C", $chk_file,"-Q",$psi_fn, "-o", $hits_file, "\n";
    ...
    system($args) == 0 or die "[$0] ERROR: $args failed: $?\n";
    if($run_only_blast) {
	print "I was only told to run blast. Not doing anything else!\n";
	exit;
    }
} else {
    print "Using the existing $mtx_fn\n";
}
#my $mtx_fn = join '/', $out_dir, $tmp_base.".mtx";
die "[$0] ERROR: Couldn't find the mtx file $mtx_fn\n" if !-e $mtx_fn;

my @exts = ("diso", "diso2", "nndiso", "dnb", "diso", "in_svm_dat", "out_svm_dat", "pbdat","ss_horiz","ss","ss2");
my ($diso_fn, $diso2_fn, $nndiso_fn, $dnb_fn, $diso3_fn, $dat_fn, $svc_fn, $pb_fn, $ss_horiz_fn,$ss_fn,$ss2_fn) = map { abs_path(join '/', $out_dir, $base.".$_") } @exts;
...
format_protein_binding_predictions($diso3_fn, $svc_fn, $pb_fn);
    ...
    print "Skipping disopred because it is already done! /BW added 20150331\n";
}
print "Predicting Secondary Structure with PSIPRED, BW added 201503016\n";

my $datadir=$DATA_DIR_PSIPRED;

$args="$EXE_DIR_PSIPRED/psipred $mtx_fn $datadir/weights.dat $datadir/weights.dat2 $datadir/weights.dat3 > $ss_fn";
print $args."\n";
system($args);
$args="$EXE_DIR_PSIPRED/psipass2 $datadir/weights_p2.dat 1 1.0 1.0 $ss2_fn $ss_fn > $ss_horiz_fn";
system($args);
...
print "Cleaning up ...\n\n";
if(-e "$tmp_base.aux") {
`rm $tmp_base.sn $tmp_base.pn $tmp_base.mn $tmp_base.aux`;  
}
